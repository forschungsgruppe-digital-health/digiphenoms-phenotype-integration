...
11. [Implementierungshinweise](#11-implementierungshinweise)
...
Die Operation stellt sicher, dass die referenzielle Integrität erhalten bleibt (Patienten vor Encounters vor Observations), organisiert Importvorgänge in einer nachvollziehbaren Group-Hierarchie und unterstützt zwei Modi: Merge (Update/Insert) und Distinct (Create).
...
### 11.2 Anpassung der Mapper-Pipeline

Der bestehende Python-Mapper (`build_bundle()`) muss um zwei Aspekte erweitert werden:

1. **Identifier-Erzeugung:** Jede Ressource erhält ein `identifier`-Element mit dem projektspezifischen OID-System.
2. **Bundle-Typ:** Der Output-Bundle-Typ wird von `transaction` auf `collection` geändert, da die Transaction-Logik in der `$cohort-submit`-Operation liegt.

### 11.3 Client-Aufruf (Python)
...
def submit_cohort(bundle_path: str, mode: str = "merge"):
    bundle = json.loads(Path(bundle_path).read_text())
    parameters = {
        "resourceType": "Parameters",
        "parameter": [
            {"name": "mode", "valueCode": mode},
            {"name": "batchLabel", "valueString": f"Import {datetime.now().isoformat()}"},
            {"name": "inputBundle", "resource": bundle}
        ]
    }
    response = httpx.post(
        f"{HAPI_BASE}/$cohort-submit",
        json=parameters,
